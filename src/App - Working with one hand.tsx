...
function App() {
  ...
  function getHand(hand: Hand, coloda: Card[], conditions: HandConditions) {
    ...
    do {
      ...
      miniColoda = [...coloda]
      for (let i = 0; i < 13; i++) {
        let index = generateRandomIndex(miniColoda);
        let element = miniColoda[index];
        hand.hand.push(element);
        miniColoda = miniColoda.filter((cardInRemainingCards) => cardInRemainingCards !== element);
      }
    } while (isValid(hand, conditions));

    return { hand: hand, remainingCards: miniColoda }
  }
  ...
  function genHand(hand: Hand) {
    ...
    let newRemainingCards = foo.remainingCards
    setHand2(foo.hand.hand)
    setRemainingCards(newRemainingCards)
    setLeft(newRemainingCards)
  }
